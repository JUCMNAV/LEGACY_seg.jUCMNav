 ...
 * Therefore, variable names retain their UCM origins.
 * 
 * @author jkealey
 * 
 */
		...
		while ((line = reader.readLine()) != null) {
			...
			if (line.matches("\\s*digraph " + AutoLayoutPreferences.MAPPREFIX + "\\d+\\s*\\{")) { //$NON-NLS-1$ //$NON-NLS-2$
				...
				if (line.matches("\\s*graph \\[bb=\"\\d+,\\d+,\\d+,\\d+\"];")) { //$NON-NLS-1$
					...
					/*
					 * if (compRef.getParent() == ParentFinder.findParent((Map)
					 * compRef.eContainer(), compRef, compRef.getX(),
					 * height-compRef.getY()-36, 54, 36)) { //Command resize =
					 * new
					 * SetConstraintBoundContainerRefCompoundCommand(compRef,
					 * compRef.getX(), compRef.getY(), 108, 72); Command resize =
					 * new
					 * SetConstraintBoundContainerRefCompoundCommand(compRef,
					 * compRef.getX(), height-compRef.getY()-36, 54, 36);
					 * cmd.add(resize); }
					 */
				}
