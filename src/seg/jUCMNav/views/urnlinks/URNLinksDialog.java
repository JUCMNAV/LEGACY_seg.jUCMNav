...
public class URNLinksDialog {
    ...
    /**
     * This method used the command stack to create a link between the grl element and the selected ucm element
     */
    private void createLink() {
        if (toElement != null) {
            AddUrnLinkCommand cmd = new AddUrnLinkCommand(urn, (URNlink) ModelCreationFactory.getNewObject(urn, URNlink.class), fromElement, toElement);
            if (cmd.canExecute()) {
                execute(cmd);
     ...
     * 
     * @param type
     *            String corresponding to the selected element type
     */
    ...
    private void refreshListLink() {
        ...
        for (Iterator it = fromElement.getFromLinks().iterator(); it.hasNext();) {
            ...
            listLinks.add(link.getToElem().getName() + " (" + link.getToElem().getId() + ")"); //$NON-NLS-1$ //$NON-NLS-2$
            urnlinks.add(link);
        }
        buttonDeleteLink.setEnabled(false);
    }
