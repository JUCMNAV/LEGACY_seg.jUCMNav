...
public class Hao2011Algorithm implements IGRLStrategyAlgorithm {
	...
	private static final String VAR_PREFIX = "Node";
    ...
    public void init(EvaluationStrategy strategy, HashMap evaluations) {
        ...
        oneHundred = new IntVar(getStore(), "oneHundred", 100, 100);
        minusOneHundred = new IntVar(getStore(), "minusOneHundred", -100, -100);
	...
	private void generateConstraints(IntentionalElement element, List<IntentionalElement> andDecompositionElements, List<IntentionalElement> orDecompositionElements,
		...
		if(andDecompositionElements.size() != 0) {
			...
			minMaxResult = new IntVar(getStore(), "minMaxResult" + element.getId(), -100, 100);
		...
		if(contributionElements.size() != 0) {
			...
			IntVar sum = new IntVar(store, "sum" + element.getId() , -10000*numberOfNodesFromContributionLinks, 10000*numberOfNodesFromContributionLinks); 
			//MAX (-100, MIN(100, (60 * A + B * -60 + c*-70) / 100 ))
			IntVar sumOver100 = new IntVar(store, "sum" + element.getId() + "Over100", -100*numberOfNodesFromContributionLinks, 100*numberOfNodesFromContributionLinks); //use collection size
			IntVar remainder = new IntVar(store, "remainder" + element.getId(), -100, 100);
			IntVar quotient = new IntVar(store, "quotient" + element.getId(), -10000*numberOfNodesFromContributionLinks, 10000*numberOfNodesFromContributionLinks);
			IntVar minCompareTo100 = new IntVar(store, "min" + element.getId() + "CompareTo100", -100*numberOfNodesFromContributionLinks, 100); 
			...
			IntVar contributionResult = new IntVar(getStore(), "contributionResult" + element.getId(), -100, 100);
	...
	private void populateSolution() {
		for (int i = 0; i < getAllElements().size(); i++) {
			...
			System.out.println("after: [" + anElement.getName() + "] with id ["+anElement.getId() +"] has evaluation ["+ getEvaluations().get(anElement).getEvaluation() +"]");
		}
		System.out.println();
	}
	...
	private void findSolution() {
    	...
    	for (int i = 0; i < fullList.length; i++) {
    		System.out.println("before: [" + fullList[i] +"]");
		}
		...
		if(isFoundSolution()) {
			for (int i=1; i<=label.getSolutionListener().solutionsNo(); i++){ 
				...
				for (int j=0; j<label.getSolution(i).length; j++) 
					System.out.print(fullList[j].id + "=["+ label.getSolution(i)[j] +"] "); 
				System.out.println(); 
			}
		}
	}
	 ...
	 * here simply because the grl models are tool generated.  
	 */
