...
import seg.jUCMNav.editors.resourceManagement.EmfModelManager;
...
public class CoreSynchronizeHelper {
	...
	public static void setUp(URNspec urn, String coreFileName, String filePath, boolean coreToJUCM) {
		...
		ResourceManager.registerExtensionFactory(EmfModelManager.CORE_FILE_EXTENSION, new CoreResourceFactoryImpl());
		...
		try {
			concern = (COREConcern) ResourceManager.loadModel(filePath + "." + EmfModelManager.CORE_FILE_EXTENSION);
			...
			if (!file.exists()) {
				...
				concern = COREModelUtil.createConcern(fileName + "." + EmfModelManager.CORE_FILE_EXTENSION); //$NON-NLS-1$
				String rootFeatureName = fileName.substring(0, 1).toUpperCase() + fileName.substring(1);
