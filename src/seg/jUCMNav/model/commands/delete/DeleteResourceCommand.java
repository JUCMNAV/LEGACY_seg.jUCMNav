...
public class DeleteResourceCommand extends Command implements JUCMNavCommand {
	 ...
	 * @param genRes
	 */
	public DeleteResourceCommand(IWorkbenchPage workbenchPage, URNspec urn,
	...
	public boolean canExecute() {
		return (urn != null)
				&& (urn.getUcmspec().getResources().contains(genRes)); // and exists resource...
	}
	...
	public void deleteDemands() {
		...
		int i = 0;
		cs = ((UCMNavMultiPageEditor) workbenchPage.getActiveEditor())
				.getDelegatingCommandStack();
		command = new CompoundCommand();
		for (Iterator dem = externalOperation.getDemands().iterator(); dem
				.hasNext();) {
			Demand aDemand = (Demand) dem.next();
			demands[i] = aDemand;
			i++;
			DeleteDemandCommand deleteCmd = new DeleteDemandCommand(urn,
					(ExternalOperation) externalOperation, aDemand, aDemand
							.getResponsibility());
	...
	public void testPreConditionsPassive() {
		assert (urn != null) && (urn.getUcmspec() != null)
				&& (urn.getUcmspec().getResources() != null) : "pre null"; //$NON-NLS-1$
	...
	public void testPostConditionsPassive() {
		assert (urn != null) && (urn.getUcmspec() != null)
				&& (urn.getUcmspec().getResources() != null) : "post null"; //$NON-NLS-1$
	...
	public void redoProcessing() {
		...
		// get a copy
		int i = 0;
		for (Iterator comp = processingResource.getComponents().iterator(); comp
				.hasNext();) {
