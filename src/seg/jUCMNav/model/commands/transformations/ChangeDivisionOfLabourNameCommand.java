 ...
 * @author jkealey
 */
public class ChangeDivisionOfLabourNameCommand extends Command implements JUCMNavCommand {
    ...
    private DivisionOfLabour lbl;
    private String name, oldName;

    public ChangeDivisionOfLabourNameCommand(DivisionOfLabour lbl, String name,ASDiagram asdiagram) {
        this.lbl = lbl;
        this.elem= lbl;
        this.spec=lbl.getAsdSpec();
        this.asdiagram=asdiagram;
        this.name = name;
        setLabel(Messages.getString("ChangeDivisionOfLabourNameCommand.changeDivisionOfLabourName")); //$NON-NLS-1$
    }
    ...
    public boolean canExecute() {
        if (elem instanceof DivisionOfLabour) {
    ...
    public DivisionOfLabour getRenamedLabel() {
        return this.lbl;
    }
    ...
    public void redo() {
        ...
        if(elem instanceof DivisionOfLabour)
    	{
    		((DivisionOfLabour)elem).setName(name);
    		((URNmodelElement) lbl).setName(name);
	...
	ArrayList al = new ArrayList(); 
        	...
        	for(Object el: spec.getOutcomes())
        	{
        		if(el instanceof Outcome && !al.contains(el)) //always true
        		{
        			...
        			{
        				((DivisionOfLabour) elem).getRequiredOutcomes().add(el);
        	...
        	for(Object el: spec.getOutcomes())
        	{
        		if(el instanceof Outcome && !al.contains(el)) //always true
        		{
        			...
        			{
        				((DivisionOfLabour) elem).getRequiredOutcomes().remove(el);
...
*/
    ...
    public void undo() {
        ...
        if(elem instanceof DivisionOfLabour)
    	{
    		((DivisionOfLabour)elem).setName(oldName);
