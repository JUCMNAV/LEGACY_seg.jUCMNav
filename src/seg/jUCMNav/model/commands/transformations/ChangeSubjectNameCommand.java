 ...
 * @author jkealey
 */
public class ChangeSubjectNameCommand extends Command implements JUCMNavCommand {
    ...
    public ChangeSubjectNameCommand(Subject lbl, String name, ASDiagram asdiagram) {
        this.lbl = lbl;
        this.elem= lbl;
        this.spec=lbl.getAsdSpec();
        this.asdiagram=asdiagram;
        this.name = name;
        setLabel(Messages.getString("ChangeSubjectNameCommand.changeSubjectName")); //$NON-NLS-1$
    }
    ...
    public Subject getRenamedLabel() {
        return this.lbl;
    }
...
ArrayList al = new ArrayList(); 
        	...
        	for(Object el: spec.getOutcomes())
        	{
        		if(el instanceof Outcome && !al.contains(el)) //always true
        		{
        			...
        			{
        				((Subject) elem).getRequiredOutcomes().add(el);
        	...
        	for(Object el: spec.getOutcomes())
        	{
        		if(el instanceof Outcome && !al.contains(el)) //always true
        		{
        			...
        			{
        				((Subject) elem).getRequiredOutcomes().remove(el);
