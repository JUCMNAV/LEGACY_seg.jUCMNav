 ...
 * 
 */
public class CreateASDiagramCommand extends Command implements JUCMNavCommand, IGlobalStackCommand {

    private URNspec urn;
    private ASDiagram graph;
    private int oldCount;
    private int index=-1;

    public CreateASDiagramCommand(URNspec urn) {
        this.urn = urn;

        // must be created here for getMap() to work properly.
        graph = (ASDiagram) ModelCreationFactory.getNewObject(urn, ASDiagram.class);
        setLabel(Messages.getString("CreateASDiagramCommand.createASDiagram")); //$NON-NLS-1$
    }
