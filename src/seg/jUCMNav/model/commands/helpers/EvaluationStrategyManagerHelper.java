...
import seg.jUCMNav.model.commands.helpers.AddEvaluationCommandHelper;
import seg.jUCMNav.model.commands.helpers.DeleteEvaluationCommandHelper;
import seg.jUCMNav.model.util.MetadataHelper;
import seg.jUCMNav.model.util.StrategyEvaluationRangeHelper;
import seg.jUCMNav.strategies.ConditionalBasedGRLStrategyAlgorithm;
import seg.jUCMNav.strategies.FeatureModelStrategyAlgorithm;
import seg.jUCMNav.strategies.FormulaBasedGRLStrategyAlgorithm;
import seg.jUCMNav.strategies.Hao2011Algorithm;
 ...
 * 
 */
public class EvaluationStrategyManagerHelper {
    ...
    private static HashMap<UCMNavMultiPageEditor, EvaluationStrategyManagerHelper> strategyManagerInstances = null;
    // just in case we're actually accessing it via some non UI thread during export and/or before the app loads.
    private static EvaluationStrategyManagerHelper noEditorStrategyManagerInstance = null;
    ...
    public static synchronized EvaluationStrategyManagerHelper getInstance(UCMNavMultiPageEditor multieditor, boolean canRefresh) {

        if (strategyManagerInstances == null) {
            strategyManagerInstances = new HashMap<UCMNavMultiPageEditor, EvaluationStrategyManagerHelper>();
        }

        EvaluationStrategyManagerHelper soleInstance = null;
        ...
        if (multieditor != null) {
            if (strategyManagerInstances.get(multieditor) == null)
                strategyManagerInstances.put(multieditor, new EvaluationStrategyManagerHelper());
            ...
            if (noEditorStrategyManagerInstance == null)
                noEditorStrategyManagerInstance = new EvaluationStrategyManagerHelper();
    ...
    public static synchronized EvaluationStrategyManagerHelper getInstance(UCMNavMultiPageEditor multieditor) {
        return getInstance(multieditor, true);
    }

    public static synchronized EvaluationStrategyManagerHelper getInstance(boolean canRefresh) {
        return getInstance(null, canRefresh);
    }

    public static synchronized EvaluationStrategyManagerHelper getInstance() {
     ...
     * 
     */
    private EvaluationStrategyManagerHelper() {
...
//        }
    ...
    public synchronized void setupEvaluationAlgorithm() {
        ...
        if ((StrategyEvaluationPreferences.MIXED_ALGORITHM + "").equals(algoChoice)) //$NON-NLS-1$
            ...
            algo = new FeatureModelStrategyAlgorithmHelper();
        else
    ...
    public synchronized void setStrategy(EvaluationStrategy strategy) {
        ...
        if (strategy != null) {
            ...
            while (it.hasNext()) {
                ...
                if (eval == null) {
                    eval = (Evaluation) ModelCreationFactoryHelper.getNewObject(grl.getUrnspec(), Evaluation.class);
                }
...
//        }
    ...
    protected synchronized void setIntentionalElementEvaluation(IntentionalElement element, int value, boolean delete) {
        ...
        if (value <= IGRLStrategyAlgorithm.SATISFICED && value >= IGRLStrategyAlgorithm.UNDECIDED) {
            ...
            if (!delete) {
                ...
                AddEvaluationCommandHelper cmd = new AddEvaluationCommandHelper(eval, element, strategy);
                execute(cmd);
            } else {
                DeleteEvaluationCommandHelper cmd = new DeleteEvaluationCommandHelper(eval);
                execute(cmd);
            }
    ...
    public static QualitativeLabel getQualitativeEvaluationForQuantitativeValue(URNspec urn, int value) {
        value = StrategyEvaluationPreferencesHelper.getEquivalentValueInFullRangeIfApplicable(urn, value);
  ...
  //  }
    ...
    private void setEvaluationMetadata(IntentionalElement element, Evaluation eval) {
        ...
        if (metaNumerical != null) {
            ...
            // Add new run-time metadata for this element
            URNspec urnSpec = element.getGrlspec().getUrnspec();
            MetadataHelperHelper.addMetaData(urnSpec, element, METADATA_NUMEVAL, numEvalAsString);
        }
        ...
        if (metaQuantitative != null) {
            ...
            // Add new run-time metadata for this element
            URNspec urnSpec = element.getGrlspec().getUrnspec();
            MetadataHelperHelper.addMetaData(urnSpec, element, METADATA_QUALEVAL, qualEvalAsString);
        }
    }
...
//        }
    ...
    public KPIEvalValueSet getActiveKPIEvalValueSet(IntentionalElement elem) {
        ...
        if (set == null) { // ensure always has a value, even if the ev is temporary  
            set = (KPIEvalValueSet) ModelCreationFactoryHelper.getNewObject(null, KPIEvalValueSet.class);
        }
        return set;
    ...
    public void setActiveKPIEvaluationValue(IntentionalElement elem, double value, boolean recomputeAll) {
        ...
        // If it is a new Evaluation enter by the user, link it with the strategy and intentionalElement
        AddEvaluationCommandHelper cmd = new AddEvaluationCommandHelper(eval, elem, strategy);
        execute(cmd);
