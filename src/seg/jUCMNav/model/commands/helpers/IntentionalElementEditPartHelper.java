...
import seg.jUCMNav.editparts.DragPathNodeTracker;
...
public class IntentionalElementEditPartHelper  {
    ...
    public static String determineColor(URNspec urn, IntentionalElement elem, Evaluation evaluation, boolean ignored, int evalType) {
        ...
        if (ignored) { // set to light gray color
            ...
            evalValue = StrategyEvaluationPreferencesHelper.getEquivalentValueInFullRangeIfApplicable(urn, evalValue);
            
            if (EvaluationStrategyManagerHelper.getInstance().displayDifferenceMode()
                    && !StrategyEvaluationPreferencesHelper.getVisualizeAsPositiveRange(urn)) {
                evalValue /= 2;
            }
