...
public class ChangeNumericalEvaluationCommandHelper implements Command {
    private int operation;
    private boolean removed = false;
    private CommandStack commandStack;
    EvaluationStrategyManagerHelper esm;
    ...
    public ChangeNumericalEvaluationCommandHelper(List intElemRefs, int id, int enteredValue, CommandStack stack) {
     //   setLabel(Messages.getString("UrnContextMenuProvider.SetNumericalEvaluation")); //$NON-NLS-1$

        int enteredEval = 0;
        operation = id;
        commandStack = stack;
        esm = EvaluationStrategyManagerHelper.getInstance();
        ...
        for (Iterator iter = intElemRefs.iterator(); iter.hasNext();) {
            ...
            if (id < USER_ENTRY) { // input from sub-menu +100 -> -100 
                ...
                ies.newEval = StrategyEvaluationPreferencesHelper.getEquivalentValueIn0To100RangeIfApplicable(urn, ies.newEval);
            }
