...
import seg.jUCMNav.editpolicies.directEditPolicy.RuleDirectEditPolicy;
import seg.jUCMNav.editpolicies.element.RuleComponentEditPolicy;
import seg.jUCMNav.figures.ASDiagramBackgroundImage;
import seg.jUCMNav.figures.AsdTextEditor;
import seg.jUCMNav.figures.RuleFigure;
import seg.jUCMNav.figures.util.UrnMetadata;
import seg.jUCMNav.views.property.LabelPropertySource;
import urncore.URNmodelElement;


public class RuleEditPart extends ModelElementEditPart {
	// label padding
	...
	public RuleEditPart(ASDmodelElement model,ASDiagram asdDiagram) {
		super();
		setModel(model);
		modelElement= model;
	...
	protected void createEditPolicies() {
		...
		installEditPolicy(EditPolicy.DIRECT_EDIT_ROLE, new RuleDirectEditPolicy(asdDiagram));
	...
	protected IFigure createFigure() {
		...
		return new RuleFigure(asdSpec,(Rule)modelElement,asdDiagram);
	...
	private boolean directEditHitTest(Point requestLoc) {
		RuleFigure figure = (RuleFigure) getFigure();
	...
	public void handleNameChange(String value) {
		RuleFigure tableFigure = (RuleFigure) getFigure();
	...
	protected void refreshVisuals() {        
		...
		if (modelElement != null) {
			eraseTargetFeedback(new SelectionRequest());
			RuleFigure labelFigure = (RuleFigure) getNodeFigure();
           ...
           //ASDiagramBackgroundImage aImage= new ASDiagramBackgroundImage("ASDiagram.gif");
			Point location= new Point(aImage.getX()-410,ycoordinate);
