 ...
 * should be associated with this EditPart.
 * 
...
public class PathNodeEditPart extends ModelElementEditPart implements NodeEditPart
{
	...
	/**
	 * Overriding because we also have to listen to the responsibility
	 * definition
	 * 
	...
	/**
	 * Mapping between PathNode and its figure. Assertion will fail if unable to
	 * find mapping.
	 * 
	...
	/**
	 * When nodes are dragged in GEF, they explictly remove connections from
	 * being possible drop targets. By overriding DragEditPartsTracker, we allow
	 * this behaviour.
	 * 
	...
	/**
	 * We must return a connection anchor or else GEF blows up. Most PathNodes
	 * don't use these.
	 * 
	 ...
	 * @param nodeFigure
	 *            to be checked
	 * @return should the figure be moved in order for its coordinates to match
	 *         the model element.
	 */
	...
	public void performRequest(Request req)
	{
		if (req.getType() == REQ_OPEN)
		{
			if (getNode() instanceof StartPoint || getNode() instanceof EndPoint)
			{
				...
				{// if (getNode() instanceof EndPoint) {
				...
				else if (activeBindings.size() > 1)
				{
					...
					dlg = new PluginListDialog(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), ((UCMConnectionOnBottomRootEditPart) getRoot()).getMultiPageEditor());
	 ...
	 * its first outgoing connection. (AndFork)
	 * 
	 * @param nodeFigure
	 *            rotate this figure
