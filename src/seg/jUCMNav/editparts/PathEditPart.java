...
import seg.jUCMNav.editpolicies.PathComponentEditPolicy;
...
public class PathEditPart extends AbstractGraphicalEditPart implements Adapter {
	...
	public void deactivate() {
		if(isActive())
			getPath().eAdapters().remove(this);
		super.deactivate();
	}
	
	protected IPropertySource getPropertySource() {
		if( propertySource == null ) {
			propertySource = new EObjectPropertySource( getPath() );
		}
...
//		installEditPolicy(EditPolicy.DIRECT_EDIT_ROLE, new PathDirectEditPolicy());
		installEditPolicy(EditPolicy.COMPONENT_ROLE, new PathComponentEditPolicy());
	}
	...
	protected List getModelChildren() {
		...
		children.add(getPath().getStartpoint());
		children.addAll(getPath().getNodes());
		children.add(getPath().getEndpoint());
		return children;
	}
...
//		Point location = new Point(((Node)getPath().getStartpoint()).getX(), ((Node)getPath().getStartpoint()).getY());  // The position of the current figure
