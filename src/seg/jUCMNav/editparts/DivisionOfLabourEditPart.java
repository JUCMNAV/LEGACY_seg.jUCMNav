...
import seg.jUCMNav.editpolicies.directEditPolicy.DivisionOfLabourDirectEditPolicy;
import seg.jUCMNav.editpolicies.directEditPolicy.ExtendedDirectEditManager;
import seg.jUCMNav.editpolicies.element.DivisionOfLabourComponentEditPolicy;
import seg.jUCMNav.figures.ASDiagramBackgroundImage;
import seg.jUCMNav.figures.AsdTextEditor;
import seg.jUCMNav.figures.DivisionOfLabourFigure;
import seg.jUCMNav.figures.util.UrnMetadata;
import seg.jUCMNav.views.property.LabelPropertySource;
import urncore.URNmodelElement;


public class DivisionOfLabourEditPart extends ModelElementEditPart {
	// label padding
	...
	public DivisionOfLabourEditPart(ASDmodelElement model,ASDiagram asdDiagram) {
		super();
		setModel(model);
		modelElement= model;
	...
	protected void createEditPolicies() {
		...
		installEditPolicy(EditPolicy.DIRECT_EDIT_ROLE, new DivisionOfLabourDirectEditPolicy(asdDiagram));
		 installEditPolicy(EditPolicy.COMPONENT_ROLE, new DivisionOfLabourComponentEditPolicy());
	...
	protected IFigure createFigure() {
		...
		return new DivisionOfLabourFigure(asdSpec,(DivisionOfLabour)modelElement,asdDiagram);
	...
	private boolean directEditHitTest(Point requestLoc) {
		DivisionOfLabourFigure figure = (DivisionOfLabourFigure) getFigure();
	...
	public void handleNameChange(String value) {
		DivisionOfLabourFigure tableFigure = (DivisionOfLabourFigure) getFigure();
	...
	protected void refreshVisuals() {        
		...
		if (modelElement != null) {
			eraseTargetFeedback(new SelectionRequest());
			DivisionOfLabourFigure labelFigure = (DivisionOfLabourFigure) getNodeFigure();
           ...
           {
        	   if(el instanceof DivisionOfLabour)
        	  count++;
        	   if(el==modelElement)
        	   {      		   
           ...
           //ASDiagramBackgroundImage aImage= new ASDiagramBackgroundImage("ASDiagram.gif");
			Point location= new Point(aImage.getX()+160,ycoordinate);
