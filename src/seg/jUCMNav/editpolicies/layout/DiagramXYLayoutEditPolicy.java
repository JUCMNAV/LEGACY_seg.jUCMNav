 ...
 * 
 */
public class DiagramXYLayoutEditPolicy extends AbstractDiagramXYLayoutEditPolicy {
     ...
     * @return the map
     */
    protected ASDiagram getDiagram() {
        return (ASDiagram) getHost().getModel();
    }
    ...
    protected Command getCreateCommand(CreateRequest request) {
      ...
      //  } else 
        	if (newObjectType == Comment.class) {
    ...
    private Command handleCreateComponentRef(CreateRequest request, Rectangle constraint) {
        ...
        AddContainerRefCommand create = new AddContainerRefCommand(getDiagram(), compRef);
    ...
    private Command handleCreateOrExtendPath(CreateRequest request, Rectangle constraint) {
     ...
     /*   if (selectedParts.size() == 1) {
            ...
            if (selected.getModel() instanceof EndPoint) {
                // extend end point
                createCommand = new ExtendPathCommand(getDiagram(), (EndPoint) selected.getModel(), constraint.x, constraint.y);
            } else if (selected.getModel() instanceof StartPoint) {
                // extend start point
                createCommand = new ExtendPathCommand(getDiagram(), (StartPoint) selected.getModel(), constraint.x, constraint.y);
            } else {
                // create new path
